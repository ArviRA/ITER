...
def main():
    ...
    start(place)
    set_max()
    choice='y'#input("Do you want to increase the radius of the search:::(y/n)")
...
def handle(request):
   ...
   if request.method=="POST":
       ...
       url = 'http://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&APPID=YOUR_&units=metric'.format(rp['lat'],rp['lon'])
       req=requests.get(url)
       data=req.json()
       data=data['name']
       place=data
